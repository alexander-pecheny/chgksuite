...
| `column_widths` | список чисел | — | Относительные ширины столбцов. Например, `1, 2, 1` делает средний столбец вдвое шире крайних |
...
### Разный текст в столбцах

По умолчанию один и тот же текст повторяется во всех ячейках сетки. Чтобы задать разный текст для каждого столбца, разделите части строкой `|||`. Количество частей должно совпадать с `columns`:
...
Если указано несколько строк (`rows`), один и тот же набор текстов повторяется в каждой строке.
