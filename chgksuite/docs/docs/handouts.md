...
- **Сжатие растровых картинок** — по умолчанию включено (`--optimize_images on`) для `hndt2pdf` и `split_fit`. Перед вёрсткой PNG/JPEG/WebP/BMP/TIFF/GIF пережимаются во временный JPEG с качеством 80, если результат меньше исходного файла. Векторные картинки не меняются. Чтобы отключить: `--optimize_images off`.
