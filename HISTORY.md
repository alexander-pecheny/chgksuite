...
- Генерация раздаток (`handouts`) переведена с TeX/tectonic на [Typst](https://typst.app). Раздатки собираются примерно в 5 раз быстрее (а первый запуск — в десятки раз быстрее, так как Typst не качает гигантский набор пакетов, как tectonic). Команда `handouts install` теперь ставит `typst`, флаг `--tectonic_package_regex` переименован в `--typst_package_regex`.
- Шрифт по умолчанию для раздаток — Noto Sans, он встроен в пакет (поддержка кириллицы из коробки, ставить шрифты в систему не нужно).
